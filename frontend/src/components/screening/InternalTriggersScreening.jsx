...
import { INTERNAL_TRIGGER_LABELS, RATING_OPTIONS, formatTriggerName } from '../../utils/triggerLabels'

/**
 * Internal Triggers Screening (Step 2 of Triggers)
 *
 * WHO ERS 3A/3B: 36 emotional triggers displayed in a responsive grid.
...
const triggerIds = Object.keys(INTERNAL_TRIGGER_LABELS)
...
const InternalTriggersScreening = ({ onComplete, onBack, initialData = {} }) => {
  // ratings: { trigger_id: 0|1|2|3 }
  const [ratings, setRatings] = useState(initialData?.internal_ratings || {})
  const [customTriggers, setCustomTriggers] = useState(
    initialData?.internal_custom?.join(', ') || ''
  )
  const [showCustom, setShowCustom] = useState(!!initialData?.internal_custom?.length)
  ...
  const handleContinue = () => {
    if (checkedCount === 0) {
      const skip = window.confirm(
        'You haven\'t selected any internal triggers. Would you like to skip? Press OK to skip or Cancel to go back.'
      )
      if (!skip) return
    }
    ...
    onComplete({
      internal_ratings: cleanRatings,
      internal_custom: customList,
    })
  }
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden animate-fade-in">
        ...
        <div className="bg-gradient-to-r from-purple to-blue p-6 sm:p-8">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-white/70 uppercase tracking-wide">
              Triggers Assessment — Part 2 of 2
            </span>
            {checkedCount > 0 && (
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-white leading-relaxed">
            Internal Triggers — Emotions & Feelings
          </h2>
          <p className="text-white/80 text-sm mt-2 max-w-2xl">
            Check each emotion or feeling that has been associated with your substance use,
            then rate how likely you are to use when experiencing that feeling.
          </p>
        </div>
          ...
          {!showCustom ? (
            ...
            <div className="animate-slide-down">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Other emotions or feelings (comma-separated):
              </label>
              <input
                type="text"
                value={customTriggers}
                onChange={(e) => setCustomTriggers(e.target.value)}
                placeholder="e.g. Hopeless, Nostalgic"
        ...
        <div className="mx-4 sm:mx-6 mb-4 p-4 bg-blue/5 border border-blue/20 rounded-lg">
          <p className="text-sm text-gray-700">
            <strong>Note:</strong> Select all emotions that apply to you and rate each one.
...
export default InternalTriggersScreening