...
/**
 * Generate Clean, Professional PDF report for validated screening results
...
export const generateScreeningPDF = async (results) => {
  ...
  // Brand Color Palette - Purple is primary
  const colors = {
    primary: [91, 45, 145],        // Purple #5B2D91 (Main)
    red: [229, 57, 53],            // Red #E53935
    blue: [47, 128, 195],          // Blue #2F80C3
    green: [124, 179, 66],         // Green #7CB342
    orange: [242, 153, 46],        // Orange #F2992E
    darkText: [51, 51, 51],        // Dark text
    lightText: [120, 120, 120],    // Light text
    border: [224, 224, 224],       // Light border
    lightBg: [250, 250, 250],      // Very light background
    lightPurple: [243, 237, 250],  // Light purple background
    white: [255, 255, 255]
  }
  ...
  // Helper for clean section titles
  ...
  const addRiskBadge = (label, level, score = null) => {
    ...
    const badgeText = score ? `${level.toUpperCase()} (${score})` : level.toUpperCase()
  ...
  // Helper for simple divider line
  const addDivider = () => {
    checkNewPage(8)
    yPosition += 3
  ...
  const drawTable = (headers, rows, colWidths) => {
    ...
    // Header row - using primary purple
    ...
    rows.forEach((row, rowIndex) => {
      ...
      row.forEach((cell, i) => {
        // If it's a colored badge cell (object with text and color)
        if (typeof cell === 'object' && cell.text && cell.color) {
  ...
  // Extract data
  const { assist_results, pgsi_results, phq9_results, triggers_results, crisis_detected, demographics, ai_feedback } = results

  // ============================================================================
  // HEADER - Purple with VibeCheck logo
  // ============================================================================
  doc.setFillColor(...colors.primary)
  doc.rect(0, 0, pageWidth, 52, 'F')

  // Load and add VibeCheck logo on white background for visibility
  try {
    const logoBase64 = await loadImageAsBase64('/vibeCheck_logo.png')

    // White rounded background pad so logo colors are visible against purple
    ...
    // Add logo image centered on the white pad
    doc.addImage(logoBase64, 'PNG', whiteX + padX, whiteY + padY, logoImgWidth, logoImgHeight)
  } catch {
    // Fallback: text-based header if logo fails to load
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(24)
    doc.setFont('helvetica', 'bold')
  ...
  // "Screening Results" subtitle in white
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  ...
  // ============================================================================
  // DEMOGRAPHICS - Simple inline format
  // ============================================================================
  if (demographics) {
  ...
  // ============================================================================
  // ASSIST RESULTS - Clean summary + table
  // ============================================================================
  if (assist_results) {
    ...
    // Substance scores table
    if (assist_results.scores && Object.keys(assist_results.scores).length > 0) {
      const substancesWithScores = Object.entries(assist_results.scores)
      ...
      if (substancesWithScores.length > 0) {
        ...
        const rows = substancesWithScores.map(([substance, data]) => {
          ...
          if (substance === 'other' && assist_results.other_specify) {
            const custom = assist_results.other_specify.split(',').map(s => s.trim()).filter(s => s)
          ...
          return [
            substanceName,
            data.score.toString(),
            { text: data.risk_level.toUpperCase(), color: getRiskColor(data.risk_level) },
  ...
  if (phq9_results) {
    ...
    // Score box with light purple background
    ...
    // Severity info next to score
    doc.setFontSize(12)
    doc.setTextColor(...colors.darkText)
    doc.setFont('helvetica', 'bold')
    doc.text(`${severity.replace(/_/g, ' ').toUpperCase()} Depression`, margin + 60, yPosition + 10)

    if (phq9_results.severity_description) {
      doc.setFontSize(9)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...colors.lightText)
      const descLines = doc.splitTextToSize(phq9_results.severity_description, contentWidth - 65)
    ...
    if (phq9_results.suicidal_ideation || phq9_results.suicide_risk) {
      yPosition += 3
      doc.setFillColor(254, 226, 226)
      doc.roundedRect(margin, yPosition, contentWidth, 10, 2, 2, 'F')
      doc.setFontSize(9)
  ...
  if (pgsi_results) {
    ...
    if (pgsi_results.risk_description) {
      ...
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...colors.lightText)
      const descLines = doc.splitTextToSize(pgsi_results.risk_description, contentWidth)
  ...
  // ============================================================================
  // TRIGGERS ASSESSMENT - Clean tables
  // ============================================================================
  if (triggers_results && triggers_results.total_triggers > 0) {
    addSectionTitle('Triggers Assessment', colors.primary)

    // Summary
    addText(`You identified ${triggers_results.total_triggers} triggers: ${triggers_results.external_count} situational and ${triggers_results.internal_count} emotional.`, 10, colors.darkText)
    yPosition += 5

    // Level colors for triggers
    const levelConfig = {
    ...
    // Helper to render trigger table
    const renderTriggerTable = (title, verdicts) => {
      if (!verdicts || verdicts.length === 0) return

      addSubsectionTitle(title)
      yPosition += 3

      const headers = ['Risk Level', 'Trigger']
      const colWidths = [40, contentWidth - 40]

      const rows = []
      verdicts.forEach(verdict => {
        const config = levelConfig[verdict.level] || levelConfig.never_use
        verdict.triggers.forEach((triggerId, idx) => {
          rows.push([
            idx === 0 ? { text: config.label.toUpperCase(), color: config.color } : '',
    ...
    renderTriggerTable('Situational Triggers (External)', triggers_results.external_verdicts)
    renderTriggerTable('Emotional Triggers (Internal)', triggers_results.internal_verdicts)
  }

  // ============================================================================
  // AI PERSONALIZED FEEDBACK - Clean format
  // ============================================================================
  if (ai_feedback) {
  ...
  // ============================================================================
  // CRISIS RESOURCES - New page, clean format
  // ============================================================================
  doc.addPage()
  yPosition = margin

  // Resources header - Purple primary
  doc.setFillColor(...colors.primary)
  ...
  hotlines.forEach(h => {
    addKeyValue(h.type, h.number, colors.primary)
  })

  yPosition += 5

  // Check if risk is moderate or high - show hospitals if so
  const assistRisk = assist_results?.overall_risk || 'low'
  const phq9Severity = phq9_results?.severity || 'minimal'
  const showHospitals = ['moderate', 'high'].includes(assistRisk) ||
  ...
  if (showHospitals) {
    ...
    const typeColors = {
      ...
      Rehabilitation: colors.green || [124, 179, 66],
    }
    ...
    yPosition += 5
    addText('Federal = Federal Neuro-Psychiatric Hospitals | Teaching = University Teaching Hospitals | State = State Psychiatric Hospitals | Rehabilitation = Nigeria Counselling & Rehabilitation Centres', 7, colors.lightText)
  }
  ...
  notes.forEach(note => {
    addText('• ' + note, 9, colors.lightText)
  })
